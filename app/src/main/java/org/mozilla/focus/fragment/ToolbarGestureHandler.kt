...
import org.mozilla.focus.utils.getRectWithScreenLocation
import org.mozilla.focus.utils.getWindowInsets
import org.mozilla.focus.utils.isKeyboardVisible
...
class ToolbarGestureHandler(
    ...
    override fun onSwipeStarted(start: PointF, next: PointF): Boolean {
        ...
        return !activity.window.decorView.isKeyboardVisible() &&
        start.isInToolbar() &&
        abs(dx) > touchSlop &&
        abs(dy) < abs(dx)
    }
